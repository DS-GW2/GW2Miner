...
namespace GW2Miner
{
    static class Program
    {
        ...
        {
            List<Item> sellItemList = trader.get_my_sells().Result;
            Task<List<Item>> itemList = trader.get_my_buys();
            double BLSalvageCost = trader.BlackLionKitSalvageCost;  // assume BLSK
            ...
            foreach (Item item in itemList.Result)
            {
                ...
                Item richItem = trader.add_GW2DB_data(item);
                ...
                if (richItem.TypeId == TypeEnum.Armor)
                {
                    ...
                    worth = trader.Worth(item, sellItemList, runesCollection, BLSalvageCost);
                }
                else if (richItem.TypeId == TypeEnum.Weapon)
                {
                    ...
                    worth = trader.Worth(item, sellItemList, sigilsCollection, BLSalvageCost);
                }
                else
                {
                    worth = trader.Worth(item, sellItemList, null, BLSalvageCost);
                }
                ...
                if (listing.PricePerUnit > item.UnitPrice)
                {
                    ...
                    Item myItemOnSale;
                    int sellPrice = trader.GetMySellPrice(sellItemList, item, out IAmSelling, out underCut, out myItemOnSale);
